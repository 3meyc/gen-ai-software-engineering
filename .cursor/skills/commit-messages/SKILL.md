...
|--------|-----------------|
| Homework deliverables under `homework-N/` | `homework-N` or path segment (`homework-4/agents`) |
| Course repo root docs (`README.md`, `TASKS.md`) | omit scope or `docs` |
| Shared `.cursor/skills` or rules | `cursor` or `chore` |
| Hono/Vitest backend work (when present) | service or package name (`bff`, `api`) |

Match recent `git log` tone; this repo often uses `feat:` for homework tasks and `docs:` for spec/README updates.
...
| Vague subjects (`update files`, `misc changes`) | Specific outcome (`add research-quality skill for verifier`) |
...
Use a bulleted list only if numbering is awkward. Keep a one-sentence intro before the list when helpful.
...
- Thematic `-` bullets are allowed **inside** the commit message body for multi-theme commits (see above). Numbered/bulleted lists in **chat** are only for `split-suggestion` (multiple separate commits).
